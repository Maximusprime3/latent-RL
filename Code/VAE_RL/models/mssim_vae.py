...
class MSSIMVAE(BaseVAE):
    ...
    def __init__(self,
        ...
        #i changed this from *4 to *2
        #self.fc_mu = nn.Linear(hidden_dims[-1]*2, 3)
        #self.fc_var = nn.Linear(hidden_dims[-1]*2, 3)
        ...
        #i changed from nn.Tanh() to nn.Sigmoid()
    ...
    def loss_function(self,
        ...
        #betas=(0.0448, 0.2856, 0.3001, 0.2363, 0.1333) for image size 160
        #print("input", input.shape)
        #print("recons", recons.shape)
        #print(recons.unique())
        #make recons positive
        #recons = (recons**2)**0.5
        #recons_loss = multiscale_structural_similarity_index_measure(recons, input, kernel_size=11, betas=(0.0448, 0.2856, 0.3001))
        
        #cross entropy loss, can have reduction='sum' to sum all the losses but mean is more comparable across different data batch sizes
        #Mountain-car-v0
...
class MSSIM(nn.Module):
    ...
    def ssim(self,
        ...
        img_range = img1.max() - img1.min() # Dynamic range otherwise 1
        C1 = (0.01 * img_range) ** 2
        C2 = (0.03 * img_range) ** 2
